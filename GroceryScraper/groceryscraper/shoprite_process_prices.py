...
class ShopriteProcessPrices:
    ...
    def process_individual_price(self, raw_price):
        processed = re.sub(r'gal|fl|oz|ea|qt|ct|lb|\(|\)|\$|/|each| |avg', "", raw_price)
        return processed
    ...
    def find_units(self, raw_unit_price):
        units = re.search(r'gal|oz|fl|qt|ct|lb|sq|each', raw_unit_price)
        unit_list = units.group()
        return unit_list

    def process(self, raw_unit_price):
        processed = re.sub(r'gal|fl|oz|ea|qt|ct|lb|\(|\)|\$|/| |each', "", raw_unit_price)
